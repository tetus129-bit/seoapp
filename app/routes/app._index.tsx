...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  try {
    const pagesResponse = await admin.graphql(
      `#graphql query getPagesSEO { pages(first: 50) { edges { node { id title handle bodySummary seoTitleTag: metafield(namespace: "global", key: "title_tag") { value } seoDescTag: metafield(namespace: "global", key: "description_tag") { value } metafield(namespace: "seo", key: "hidden") { id value } } } } }`
    );
  ...
  try {
    if (intent === "save_seo_metadata") {
      ...
      if (resourceType === "product") {
        await admin.graphql(
          `#graphql mutation updateProductSEO($input: ProductInput!) { productUpdate(input: $input) { userErrors { field message } } }`,
          { variables: { input: { id: resourceId, seo: { title: seoTitle, description: seoDesc } } } }
        );
      } else if (resourceType === "collection") {
        await admin.graphql(
          `#graphql mutation updateCollectionSEO($input: CollectionInput!) { collectionUpdate(input: $input) { userErrors { field message } } }`,
          { variables: { input: { id: resourceId, seo: { title: seoTitle, description: seoDesc } } } }
        );
      } else if (resourceType === "page" || resourceType === "article") {
        await admin.graphql(
          `#graphql mutation updateContentSEO($metafields: [MetafieldsSetInput!]!) { metafieldsSet(metafields: $metafields) { userErrors { field message } } }`,
    ...
    if (intent === "toggle_sitemap") {
      ...
      await admin.graphql(
        `#graphql mutation setSEOHiddenMetafield($metafields: [MetafieldsSetInput!]!) { metafieldsSet(metafields: $metafields) { userErrors { field message } } }`,
    ...
    if (intent === "bulk_update_alt_texts") {
      ...
      const productsQuery = await admin.graphql(
        `#graphql query getMissingAltImages { products(first: 25) { edges { node { id title media(first: 10) { nodes { ... on MediaImage { id image { altText } } } } } } } } }`
      );
      ...
      for (const edge of products) {
        ...
        if (missingAltNodes.length > 0) {
          ...
          await admin.graphql(
            `#graphql mutation updateMediaAlt($media: [UpdateMediaInput!]!, $productId: ID!) { productUpdateMedia(media: $media, productId: $productId) { mediaUserErrors { field message } } }`,
